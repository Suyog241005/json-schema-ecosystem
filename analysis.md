  ...
  collect-metrics:
    ...
    steps:
      - name: Checkout Repository
        ...
        with:
          fetch-depth: 0 # Better for git history and pushing
      ...
      - name: Run Insights Collection script
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: npm run metrics:insights
      ...
      - name: Commit and Push Results
        run: |
          ...
          if git diff --staged --quiet; then
            ...
            git commit -m "chore: update ecosystem metrics (Phase 1 & Insights) $(date -u +%Y-%m-%d)"
      ...
      - name: Notify on Failure
        ...
        run: |
          gh issue create \
            ...
            --body "The weekly metrics collection failed on $(date). Please check the [workflow logs](https://github.com/${{ github.repository }}/actions/runs/${{ github.run_id }}) for details." \
            --label "bug,automated"
```
...
The current implementation already writes files in the data folder `(data/snapshots/metrics-YYYY-MM-DD.json)`, `(data/bowtie/snapshots/bowtie-scores-YYYY-MM-DD.json)` and `(data/insights/insights-YYYY-MM-DD.json)` each week, creating a queryable historical record. To ensure reliability against GitHub's aggressive API limits, the automation uses a centralized, throttled `Octokit` client that automatically handles retries and backoffs.
